...
## How many rows have NA in visual but has a result in qPCR? 1827 rows, quite a lot. 
lack_visual_subset <- filter(dragon_disease, is.na(dragon_disease$Diseased.Visual) & (!is.na(dragon_disease$qPCR.Result)))

## Exclude these rows for now: 
dragon_disease <- filter(dragon_disease, !(is.na(dragon_disease$Diseased.Visual) & (!is.na(dragon_disease$qPCR.Result))))
...
for (i in 1:nrow(dragon_disease)) {
    ...
    if ((dragon_disease$Diseased.Visual[i] == 0) & is.na(dragon_disease$qPCR.Result[i])) {
...
length(which(dragon_disease$Diseased_sum == "Healthy")) # 45831
length(which(dragon_disease$Diseased_sum == "Symptomatic")) # 1020
length(which(dragon_disease$Diseased_sum == "Asymptomatic")) # 946
# Only about 4% are diseased? 
# Double check NA just in case: all rows had been assigned a disease status
length(which(is.na(dragon_disease$Diseased_sum))) # 0

## Separate date/month/year for both dragon_disease and dragon_physio

dragon_physio$Catch.date <- as.Date(dragon_physio$Catch.date, "%d/%m/%Y")

dragon_physio <- dragon_physio |> transform(catch_day = format(Catch.date, "%d"), 
          catch_month = format(Catch.date, "%m"), catch_year = format(Catch.date, "%Y"))

dragon_disease$Date <- as.Date(dragon_disease$Date, "%d/%m/%Y")

dragon_disease <- dragon_disease |> transform(catch_day = format(Date, "%d"), 
                                     catch_month = format(Date, "%m"), catch_year = format(Date, "%Y"))

## Clean up dragon_disease to include only useful columns:
dragon_disease <- dragon_disease |> select(Name, Diseased_sum, catch_year)
# Remove any duplicate: i.e. Same individual, same year, same disease status
dragon_disease <- dragon_disease[!duplicated(dragon_disease), ]

## Checking how many individuals have their disease status changed within a year
## What to match??




##Make a left join one:
dragon_full_left = left_join(dragon_physio, dragon_disease, 
                    by = c("Name" = "Name", "catch_year" = "catch_year", 
